...
export default function Clientes() {
  ...
  const clientesFiltrados = useMemo(() => {
    ...
    return clientes.filter(cl => !q || (cl.cedula + ' ' + cl.nombre).toLowerCase().includes(q));
  }, [clientes, buscar]);
      ...
      <div style={{ fontFamily: "var(--font-sans, system-ui, -apple-system, sans-serif)", padding: "10px 0" }}>
        ...
        <div className="ma-card">
            ...
            <table className="ma-table">
              ...
              <tbody>
                {clientesFiltrados.length === 0 ? (
                  <tr>
                    <td colSpan="7" style={{ textAlign: "center", padding: "40px", color: "var(--merco-muted)" }}>
                  ...
                  clientesFiltrados.map(cl => {
                    ...
                    const ultimoCaso = casosCliente.slice().sort((a, b) => b.opened - a.opened)[0];

                    return (
                      <tr key={cl.cedula}>
                        <td>
                          <b style={{ color: "var(--merco-text)" }}>{cl.cedula}</b>
                        </td>
                        <td>
                        ...
                        <td style={{ color: "var(--merco-muted)" }}>{cl.tel || '—'}</td>
                        <td style={{ color: "var(--merco-muted)" }}>{cl.correo || '—'}</td>
                        <td>
                          <span className="tag">
                            {casosCliente.length} {casosCliente.length === 1 ? 'caso' : 'casos'}
                          </span>
                        </td>
                        ...
                        <td>
                          <button className="btn-icon" title="Ver detalle del cliente">
                            👁️
                          </button>
                        </td>
                      </tr>
          ...
          <div style={{ padding: "12px 18px", borderTop: "1px solid var(--merco-border)", fontSize: 12.5, color: "var(--merco-muted)" }}>
            <b>{clientesFiltrados.length}</b> {clientesFiltrados.length === 1 ? 'cliente registrado' : 'clientes registrados'}
          </div>
        </div>
