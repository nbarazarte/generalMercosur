...
const PerfilInversion = ({ expandir }) => {
  ...
  const [listaPerfilRiesgo, setListaPerfilRiesgo] = useState([]);
  const [listaTieneExperiencia, setListaTieneExperiencia] = useState([]);
  const [listaObjetivoInversion, setListaObjetivoInversion] = useState([]);

  const [formSuccess, setFormSuccess] = useState(false);
  const [formSuccessMessage, setFormSuccessMessage] = useState();
  const [isLoading, setIsLoading] = useState(false);

  const [tipo_perfil_inversion_id, setTipo_perfil_inversion_id] = useState(
    plantilla?.perfil_riesgo || "",
  );
  const [tiene_experiencia_id, setTiene_experiencia_id] = useState(
    plantilla?.tiene_experiencia || "",
  );
  const [objetivo_inversion_id, setObjetivo_inversion_id] = useState(
    plantilla?.objetivo_inversion || "",
  );
        ...
        const [resMaestros] = await Promise.all([
          axios.get(`${url}datos_maestro`, {
            ...
            params: {
              ...
              str_tipo: "tiene_experiencia,perfil_riesgo,objetivo_inversion",
            },
          }),
        ]);

        // 2. Desestructuramos el objeto que devuelve el endpoint agrupado
        const { tiene_experiencia, perfil_riesgo, objetivo_inversion } =
          resMaestros.data;

        // 3. Actualizamos los estados
        setListaPerfilRiesgo(perfil_riesgo || []);
        setListaTieneExperiencia(tiene_experiencia || []);
        setListaObjetivoInversion(objetivo_inversion || []);
      } catch (error) {
  ...
  const handleChangeTipo_perfil_inversion_id = async (event) => {
    const idSeleccionado = event.target.value;
    setTipo_perfil_inversion_id(idSeleccionado);
  };

  const handleChangeTiene_experiencia_id = async (event) => {
    const idSeleccionado = event.target.value;
    setTiene_experiencia_id(idSeleccionado);
  };

  const handleChangeObjetivo_inversion_id = async (event) => {
    const idSeleccionado = event.target.value;
    setObjetivo_inversion_id(idSeleccionado);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (!tipo_perfil_inversion_id) {
      nuevosErrores.push("Perfil de riesgo.");
      isValid = false;
      setOpen(true);
    }

    if (!tiene_experiencia_id) {
      nuevosErrores.push("Tiene experiencia.");
      isValid = false;
      setOpen(true);
    }

    if (!objetivo_inversion_id) {
      nuevosErrores.push("Objetivo de inversión.");
      isValid = false;
      setOpen(true);
    }
    ...
    try {
      ...
      const [respuesta] = await Promise.all([
        axios.post(`${url}fichas/perfilInversion`, datosContacto, {
          headers: {
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
                <FormControl variant="filled" sx={{ minWidth: 120 }}>
                  <InputLabel id="tipo-label">Perfil de Riesgo</InputLabel>
                  <Select
                    labelId="tipo-label"
                    id="tipo_perfil_inversion_id"
                    name="tipo_perfil_inversion_id"
                    value={
                      listaPerfilRiesgo.length > 0
                        ? tipo_perfil_inversion_id
                        : ""
                    }
                    onChange={handleChangeTipo_perfil_inversion_id}
                  >
                    <MenuItem value="">
                      <em>Seleccione</em>
                    </MenuItem>
                    {listaPerfilRiesgo.map((option) => (
                ...
                <FormControl variant="filled" sx={{ minWidth: 120 }}>
                  <InputLabel id="tipo-label">Tiene Experiencia</InputLabel>
                  <Select
                    labelId="tipo-label"
                    id="tiene_experiencia_id"
                    name="tiene_experiencia_id"
                    value={
                      listaTieneExperiencia.length > 0
                        ? tiene_experiencia_id
                        : ""
                    }
                    onChange={handleChangeTiene_experiencia_id}
                  >
                    <MenuItem value="">
                      <em>Seleccione</em>
                    </MenuItem>
                    {listaTieneExperiencia.map((option) => (
                      <MenuItem key={option.id} value={option.id}>
                        {option.str_nombre}
                      </MenuItem>
                    ))}
                ...
                <FormControl variant="filled" sx={{ minWidth: 120 }}>
                  <InputLabel id="tipo-label">Objetivo de Inversión</InputLabel>
                  <Select
                    labelId="tipo-label"
                    id="objetivo_inversion_id"
                    name="objetivo_inversion_id"
                    value={
                      listaObjetivoInversion.length > 0
                        ? objetivo_inversion_id
                        : ""
                    }
                    onChange={handleChangeObjetivo_inversion_id}
                  >
                    <MenuItem value="">
                      <em>Seleccione</em>
                    </MenuItem>
                    {listaObjetivoInversion.map((option) => (
                      <MenuItem key={option.id} value={option.id}>
                        {option.str_nombre}
                      </MenuItem>
                    ))}
...
export default PerfilInversion;
