...
const RegistrosNoticiasCrimen = () => {
  ...
  const [openMoviendoRechazados, setOpenMoviendoRechazados] =
  ...
  const handleValidarTodoPagina = async () => {
    ...
    if (idsParaAprobar.length === 0) return alert("No hay datos");
    const confirmar = window.confirm(
      `¿Confirma que revisó la(s) ${idsParaAprobar.length} ficha(s) en Noticias Crimen y desea enviarla(s) a Fichas aprobadas?     `,
    );
    if (!confirmar) return;
    try {
  ...
  const handleValidar = async (ficha_id) => {
    const respuestaUsuario = window.confirm(
      "¿Confirma que revisó la ficha en Noticias Crimen y desea enviarla a Firma?",
    );

    if (respuestaUsuario) {
      try {
        await axios.post(
          `${url}fichas/enviarafirmadesdenoticiascrimen`,
          {
            bol_verificacion_agile_check: true,
  ...
  const handleCloseMoverRechazados = () => {
    setOpenMoviendoRechazados(false);
  };

  const handleEnviarCorreoRechazadas = async (correo, fichas_id) => {
    const respuestaUsuario = window.confirm(
      "¿Confirma que revisó esta ficha de notitia criminis y desea enviarla a fichas rechazadas?",
    );
    ...
    if (respuestaUsuario) {
      ...
      try {
        setOpenMoviendoRechazados(true);
        const response = await axios.post(
          `${url}notificar-cliente-rechazadas`,
          { email: emails }, // Envía el array de correos
          { headers },
        );

        if (response.status === 200) {
          //mueve las fichas a Fichas Rechazadas
          handleRechazar(fichas_id);

          setOpenMoviendoRechazados(false);
          setListaCorreos([]);
        }
      } catch (error) {
        console.error("Error al enviar la notificación de Rechazados:", error);
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}registrosnoticiascrimenExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
      ...
      saveAs(
        ...
        `FichasDeNoticiasCrimen_${Date.now()}.xlsx`,
      );
    } catch (error) {
      console.error(error);
    }
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}registrosnoticiascrimen`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
    ...
    <Box sx={{ width: "100%", p: 2 }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Enviar todas las fichas para firmar">
            <Button
              variant="contained"
              color="success"
              onClick={handleValidarTodoPagina}
            >
      ...
      <StyledMenu anchorEl={anchorEl} open={open2} onClose={handleClose}>
        ...
        <MenuItem
          onClick={() => {
            handleEnviarCorreoRechazadas(selectedRow?.correo, selectedRow?.id);
            handleClose();
          }}
        >
          <DeleteForeverIcon />
          ...
          >
            ...
            <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
              ...
              <Button
                ...
                size="small"
                onClick={() =>
                  handleClickOpenObservaciones(modalData?.id, modalData?.correo)
                }
              >
      ...
      {/* Enviando correo a Rechazados*/}
      <Fragment>
        <Dialog
          open={openMoviendoRechazados}
          onClose={handleCloseMoverRechazados}
          fullWidth
          maxWidth="md"
          slotProps={{
            paper: {
...
export default RegistrosNoticiasCrimen;
