...
const FichasRechazadas = () => {
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}fichasRechazadasExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}fichasRechazadas`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
...
export default FichasRechazadas;
