...
const FichasPorRevision = () => {
  ...
  const handleVerificacionDatos = async (ficha_id) => {
    const respuestaUsuario = window.confirm(
      "¿Confirma que revisó la ficha y los documentos están correctos?",
    );

    if (respuestaUsuario) {
      try {
        await axios.post(
          `${url}fichas/verificaciondocumentos`,
          {
            bol_verificacion_datos: true,
            usuario_revisado_cumplimiento_id: usuarioBoId,
            ficha_id,
          },
          { headers },
        );
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}fichasPorRevisionExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
      ...
      saveAs(
        ...
        `FichasPorRevision_${Date.now()}.xlsx`,
      );
    } catch (error) {
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}fichasPorRevision`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
  ...
  return (
    <Box sx={{ width: "100%", p: 2 }}>
      ...
      <StyledMenu anchorEl={anchorEl} open={open2} onClose={handleClose}>
        ...
        <MenuItem
          onClick={() => {
            handleVerificacionDatos(selectedRow?.id);
            handleClose();
          }}
        >
          <CheckIcon /> Enviar a fichas LC/FT – Listas y Noticias
        </MenuItem>
...
export default FichasPorRevision;
