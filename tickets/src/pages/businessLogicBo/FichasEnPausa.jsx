...
const FichasEnPausa = () => {
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}fichasenpausa`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}fichasenpausaExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
      ...
      XLSX.writeFile(workbook, `FichasEnPausa_${Date.now()}.xlsx`);
    } catch (error) {
  ...
  const handleClickOpenRedactarCorreo = async () => {
    ...
    const respuestaUsuario = window.confirm(
      clientesSeleccionados.length === 0
        ? `¿Confirma que desea crear un correo masivo para los ${totalCount} clientes de Fichas en Pausa?`
