...
const FichasActivas = () => {
        ...
        const preRegistro = filas.map((row) => {
          ...
          return {
            ...
            usuariosSolicitud: [
              {
                esJuridico: false,
                idUsuario: row.cedula.substring(1),
                groupId: 1,
  ...
  const handleEnviarFichasFirmadas = async () => {
    if (!clientesSeleccionados.length) {
      alert("Debe seleccionar al menos un cliente, intente nuevamente.");
      return;
    }

    const respuestaUsuario = window.confirm(
      "¿Confirma que desea mover los clientes seleccionados a Fichas Firmadas?",
    );

    if (respuestaUsuario) {
      try {
        // Extraemos solo los IDs aquí:
        const idsSeleccionados = clientesSeleccionados.map(
          (cliente) => cliente.id,
        );

        const response = await axios.post(
          `${url}moverFichasFirmadas`,
          {
            clientesSeleccionados: idsSeleccionados, // Enviamos el array de strings
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}fichasActivasExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}fichasActivas`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
    ...
    <Box sx={{ width: "100%", p: 2 }}>
      <Typography variant="h6" sx={{ mb: 3, fontWeight: "bold" }}>
        ...
        >
          <Box component="span" sx={{ ml: 4 }}>
            Fichas Activas Pendientes por Firmar
          </Box>
        </Badge>
      </Typography>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Pre Registrar y Enviar a Fichas Firmadas">
            <Button
              id="demo-customized-button"
      ...
      >
        ...
        >
          ...
          />
          <span>Enviar a Fichas Firmadas</span>
        </MenuItem>
