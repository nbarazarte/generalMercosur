...
const RegistrosPep = () => {
  ...
  const handleValidarTodoPagina = async () => {
    ...
    const confirmar = window.confirm(
      `¿Confirma que revisó la(s) ${idsParaAprobar.length} ficha(s) de PEP y desea enviarla(s) a Fichas aprobadas?     `,
    );
    if (!confirmar) return;
    try {
  ...
  const handleValidar = async (ficha_id) => {
    const respuestaUsuario = window.confirm(
      "¿Confirma que revisó la ficha de PEP y desea enviarla a Fichas aprobadas?",
    );

    if (respuestaUsuario) {
      try {
        await axios.post(
          `${url}fichas/enviarafirmadesdepep`,
          {
            bol_verificacion_agile_check: true,
  ...
  const exportToExcelTodo = async () => {
    try {
      const response = await axios.get(`${url}registrospepExcel`, {
        headers,
        params: {
          page,
          limit: rowsPerPage,
      ...
      saveAs(
        ...
        `FichasPEPenAgileCheck_${Date.now()}.xlsx`,
      );
    } catch (error) {
      console.error(error);
    }
  ...
  useEffect(() => {
    const fetchDocumentos = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${url}registrospep`, {
          headers,
          params: {
            page,
            limit: rowsPerPage,
