...
import TituloAppBo from "../../../components/TituloAppBo";
...
export default function SignUp(props) {
  ...
  const validateInputs = () => {
    ...
    if (!tokenUrl) {
      ...
      if (!cleanedEmail) {
        setEmailError(true);
        setEmailErrorMessage("El correo es obligatorio.");
        isValid = false;
      } else if (!/^\S+@\S+\.\S+$/.test(cleanedEmail)) {
        setEmailError(true);
        setEmailErrorMessage("Dirección de correo electrónico inválida.");
        isValid = false;
      } else if (!cleanedEmail.endsWith("@mercosur.com.ve")) {
        setEmailError(true);
        setEmailErrorMessage("Solo se permiten correos @mercosur.com.ve");
        isValid = false;
      } else {
        setEmailError(false);
    ...
    try {
      if (tokenUrl) {
        ...
        const response = await axios.post(
          `${url}register-bo`,
          { token: tokenUrl, password, username },
          { headers },
        );

        setSuccess("Registro exitoso. Redirigiendo...");

        localStorage.setItem("bo_userId", String(response.data.id));
        localStorage.setItem("bo_token", response.data.token);
        localStorage.setItem("bo_userName", response.data.username);
        localStorage.setItem("bo_userEmail", response.data.email);
        dispatch(setUsuarioBoId(response.data.id));
                  ...
                  {!tokenUrl && (
                    <FormControl>
                      ...
                      <TextField
                        ...
                        fullWidth
                        id="email"
                        placeholder="usuario@mercosur.com.ve"
                        name="email"
                        autoComplete="email"
                        variant="outlined"
                        error={emailError}
